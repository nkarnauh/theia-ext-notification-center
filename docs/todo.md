...
- [x] `backend/store/`, `backend/service/`
- [x] `frontend/client/`, `commands/`, `toast/`, `panel/`
- [x] DI-модули в корне слоёв, `theiaExtensions` без изменений
- [x] Дерево в architecture.md совпадает с диском
- [x] **Готово:** compile, lint, Jest, e2e зелёные
